...
Allerdings müssen physikalische Beobachtungen (um die Zahlen zu generieren) in einem angemessen großen Zeitabstand zueinander erfolgen. Die Dauer der Messungen kann variieren und nur so kann ihre Unabhängigkeit gewährleistet werden. 

Dies führt häufig dazu, dass nichtdeterministische Generatoren für die Erzeugung der Initialwerte verwendet werden, aus welchen dann \textbf{deterministischen Zufallsgeneratoren}, also Algorithmen, weitere \textbf{Pseudozufallszahlen} generiert.
...
Der \textbf{Algorithmus} startet mit $N = 624$ verschiedenen Wörtern, die mit $Y_1\ ...\ Y_N$ bezeichnet werden. Die Initialwörter können von einem anderen Pseudozufallszahlengenerator generiert werden. Hierbei muss drauf geachtet werden, dass nicht zu viele Wörter mit 0 initialisiert werden. Sind die Initialwörter gewählt, berechnet der Mersenne-Twister die weiteren Pseudzufallszahlen wie folgt.
...
Um zu prüfen, ob ein Generator gleichverteilte Zufallszahlen generiert, besteht die Möglichkeit mit d-gleichverteile Folgen zu arbeiten. Es können nur eine endliche Zahlenfolge $(x_n)_{n \ge 0}$ mit einer Länge $d$ überprüft werden. Sollte die Beobachtung wiederholt werden und unabhängig sein, sollten die Teilstücke disjunkt gewählt werden. 
...
überlappend. Dabei gelten folgende Eigenschaften.

$$f(k,d,i )=\begin{cases}\tilde{x_i} \text{ überlappend}&\text{, wenn }1 \le k \le d - 1\\
