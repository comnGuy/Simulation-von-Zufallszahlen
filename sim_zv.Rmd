...
Das \textbf{Mittquadratverfahren (engl.: Middle-Square method)} ist einer der ältesten und vergleichsweise einfachsten Algorithmen zur Erzeugung gleichverteilter (Pseudo)Zufallszahlen. Er wurde 1946 von John von Neumann entwickelt. Bei dieser Methode wird ein beliebiger (meistens) achtstelliger Initialwert $x_n \in \{0,1...,10^8-1\}$ genommen und quadriert. Die mittleren acht Zieffern dieses Quadrats bilden den neuen Seed.

So können mit der Rekursionvorschift
$$x_{n+1} := mittlere\ acht\ Ziffern\ von\ x^2_n$$
immer solange Zahlen gebildet, bis das erzeugte $x_{n+1}=0$ ist.
...
$$f(k,d)=\begin{cases}\text{Teilstücke überlappend}&\text{, wenn }1 \le k \le d - 1\\
                       \text{Teilstücke disjunkt und lückenlos}&\text{, wenn }k = d\\
                       \text{Teilstücke disjunkt und gespreizt}&\text{, wenn }k > d\end{cases}$$
...
Neben den R-Standard Zufallsgeneratoren, gibt es auch die Möglichkeit eigene Funktionen zu benutzen.
